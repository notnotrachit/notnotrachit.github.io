		...
		<>
			<nav
				className={`fixed top-0 left-0 w-full px-6 md:px-12 py-6 z-50 flex justify-between items-center transition-all duration-500 ${
					scrolled
						? "py-4 backdrop-blur-xl bg-[#0f0c29]/80 border-b border-white/5"
						: "bg-transparent"
				}`}
			>
				<Link
				...
				>
					...
					<span className="absolute -bottom-1 left-0 w-0 h-[2px] bg-indigo-400 transition-all duration-300 group-hover:w-full" />
				</Link>

				{/* Desktop Menu */}
				<div className="hidden md:flex items-center gap-2 bg-white/5 px-4 py-2 border border-white/5 backdrop-blur-sm shadow-lg shadow-indigo-900/10">
					{navItems.map((item) => (
					...
					<Magnetic strength={0.2}>
						<Link
							to="/stats"
							className="font-mono text-xs uppercase tracking-widest text-indigo-300 hover:text-indigo-200 transition-colors cursor-pointer px-4 py-2 relative group"
						>
							<span className="relative z-10">Stats</span>
							<span className="absolute inset-0 bg-indigo-500/10 scale-0 transition-transform duration-300 group-hover:scale-100" />
						</Link>
					</Magnetic>
				</div>
			...
			<AnimatePresence>
				{isOpen && (
					<motion.div
						...
						className="fixed inset-0 bg-[#0f0c29] z-40 flex flex-col items-center justify-center md:hidden"
					>
						{/* Background Texture */}
						...
						<div className="flex flex-col items-center gap-8 relative z-10">
							...
							>
								<Link
									...
									className="font-display text-5xl font-bold uppercase tracking-tighter text-transparent bg-clip-text bg-gradient-to-b from-indigo-300 to-indigo-300/50 hover:to-indigo-200 transition-all"
								>
									Stats
								</Link>
							</motion.div>
