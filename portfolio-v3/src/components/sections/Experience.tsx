...
export function Experience() {
	...
	const ExperienceCard = ({ item }: { item: ExperienceItem }) => (
		...
		>
			{/* Timeline dot */}
			<div className="absolute -left-[5px] top-0 w-2.5 h-2.5 bg-background border-2 border-primary/50 group-hover:border-primary group-hover:bg-primary/20 transition-all" />
			...
			<div className="pb-10">
				...
				<div className="flex items-start gap-4 mb-3">
					...
					<div className="flex-1 min-w-0">
						<div className="flex flex-wrap items-center gap-x-3 gap-y-1">
							<h4 className="font-display text-lg font-semibold text-white group-hover:text-primary transition-colors">
								{item.company}
							</h4>
				...
				<div className="flex flex-wrap gap-2">
					{item.technologies.map((tech) => (
						<span
							key={tech}
							className="px-2 py-0.5 text-xs font-mono text-primary/80 bg-primary/5 border border-primary/10"
						>
							{tech}
						</span>
					))}
	...
	return (
		...
		>
			...
			<div className="absolute top-1/4 left-0 w-[500px] h-[500px] bg-indigo-600/20 blur-[120px] -translate-x-1/2 pointer-events-none" />
			<div className="absolute bottom-1/4 right-0 w-[400px] h-[400px] bg-purple-600/15 blur-[120px] translate-x-1/2 pointer-events-none" />
